...
useEffect(() => {
  ...
  dispatch(adminActions.read_usuarios_creados(currentPage))
  }, [dispatch]);

const input_select=useRef()
const admins=useSelector(store=>store.users.users)
const read_usuarios_creados=useSelector(store=>store.users.read_usuarios_creados)
const read_usuarios_response=read_usuarios_creados?.response
console.log(read_usuarios_creados);
const usuarios_creados_filter=Array.isArray(read_usuarios_response) ? read_usuarios_response.filter(adm=>adm.usuario_admin === select_user):[]
console.log(usuarios_creados_filter);
...
function handleNext() {
  ...
  const hasNextPage = read_usuarios_creados?.nextPage;

  if (hasNextPage) {
    dispatch(adminActions.read_usuarios_creados(nextPage)).then(() => {
...
function handlePrev() {
  if (currentPage > 1) {
    ...
    dispatch(adminActions.read_usuarios_creados(prevPage)).then(() => {
...
<div className='flex flex-col w-[90%] lg:w-[40%] h-auto bg-[white] rounded-[10px]'>
  <div className='w-full flex justify-center border-b-[2px] border-b-[black] border-solid items-center py-[1rem]'>
    <p className='font-semibold text-[1.4rem] text-center'>Registro de creación de usuarios</p>
  </div>
  <div className='flex flex-col items-center px-[1rem] gap-2 py-[1rem]'>
    <p>Cuentas con permiso de crear usuarios:</p>
    ...
    <div id="emailHelp" class="form-text">Seleccione un admin o revendedor para ver su historial de creación de usuarios.</div>
    {select_user && usuarios_creados_filter.length > 0 && (
      <div className='py-[1rem] w-full flex flex-col'>
        {usuarios_creados_filter?.map(adm=>(
          <table className='table-auto w-full'>
          <thead>
            <tr className='text-center'>
              <th className='border px-4 py-2' colSpan="2">USUARIO CREADO</th>
            </tr>
          </thead>
          <tbody>
            <tr>
            ...
            <tr>
              <td className='border px-4 py-2'>Fecha de creación:</td>
              <td className='border px-4 py-2'>{adm.fecha}</td>
            </tr>
            <tr>
              <td className='border px-4 py-2'>Hora de creación:</td>
        ...
        disabled={read_usuarios_creados?.prevPage === null} className='bg-[#1db9b9] text-white p-1 rounded-[10px] disabled:bg-[gray]'><svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="currentColor" class="bi bi-arrow-left-circle-fill" viewBox="0 0 16 16">
        <path d="M8 0a8 8 0 1 0 0 16A8 8 0 0 0 8 0m3.5 7.5a.5.5 0 0 1 0 1H5.707l2.147 2.146a.5.5 0 0 1-.708.708l-3-3a.5.5 0 0 1 0-.708l3-3a.5.5 0 1 1 .708.708L5.707 7.5z"/>
      </svg></button>
        <p>Página: {currentPage}</p>
        <button  onClick={handleNext}
          disabled={ read_usuarios_creados?.nextPage === null} className='bg-[#1db9b9] text-[white] p-1 rounded-[10px] disabled:bg-[gray]'><svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="currentColor" class="bi bi-arrow-right-circle-fill" viewBox="0 0 16 16">
